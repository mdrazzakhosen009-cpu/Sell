...
async function initDatabase() {
  ...
  // First deployment: seed a polished demo catalog into Turso so the Store is not empty.
  // The seed flag prevents deleted demo products from coming back after a restart.
  const seeded = await one('SELECT value FROM settings WHERE key=?', ['demo_seeded']);
  const productCount = await one('SELECT COUNT(*) AS c FROM products');
  if (seeded?.value !== 'true' && Number(productCount?.c || 0) === 0) {
    const demo = [
      ['Royal Banarasi Silk Saree',4850,6200,'Banarasi','royal, silk, wedding','Rich Banarasi silk with an elegant zari border.',1,1,18,'product1.jpg'],
      ['Pure Katan Silk Saree',5250,6900,'Katan Silk','katan, silk, premium','Pure Katan silk crafted for timeless festive style.',1,0,16,'product2.jpg'],
      ['Soft Silk With Zari Work',3450,4300,'Soft Silk','soft silk, zari, festive','Soft silk saree with refined zari work and a graceful drape.',1,1,24,'product3.jpg'],
      ['Designer Party Wear Saree',4150,5200,'Designer','party wear, designer','A statement party-wear saree with a sophisticated black-gold finish.',1,0,14,'product4.jpg'],
      ['Traditional Kanjivaram Saree',6250,7800,'Kanjivaram','kanjivaram, traditional, silk','Traditional Kanjivaram-inspired silk styling for weddings and celebrations.',1,1,12,'product5.jpg']
    ];
    for (const d of demo) {
      const asset = path.join(__dirname, 'public', 'assets', d[9]);
      let image='';
      try { image=`data:image/jpeg;base64,${require('fs').readFileSync(asset).toString('base64')}`; } catch {}
    ...
    await exec('INSERT INTO agents(name,phone,whatsapp,messenger,active,created_at,updated_at) VALUES(?,?,?,?,?,?,?)', ['SAREE Support','01700000000','8801700000000','',1,now(),now()]);
    await exec('INSERT INTO agents(name,phone,whatsapp,messenger,active,created_at,updated_at) VALUES(?,?,?,?,?,?,?)', ['Premium Collection Desk','01800000000','8801800000000','',1,now(),now()]);
    await exec('UPDATE settings SET value=?,updated_at=? WHERE key=?', ['true',now(),'demo_seeded']);
    console.log('Demo catalog seeded into Turso.');
  }
}
...
async function groq(messages, vision=false) {
  ...
  const model = env('GROQ_MODEL') || (vision ? 'llama-3.2-11b-vision-preview' : 'llama-3.3-70b-versatile');
